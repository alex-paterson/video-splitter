...
- **Caption pipeline:** `transcribe-source` (on the cut MP4) → `transcript-to-words` → `caption-plan` (deterministic; style preset + overrides; builds phrases from words; optional `--banner <png>` + alignment/scale flags; optional `--title`) → `video-caption-render` (single Remotion pass burns captions + optional title + optional banner). Refine via `caption-refine` → writes `.caption.N.json`, then re-render.
