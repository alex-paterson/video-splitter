...
- **Transcribe cut MP4s for captions.** For post-processing (captions), transcribe the cut MP4 directly with `transcribe-source` then convert with `transcript-to-words`. This gives Whisper timings aligned to the actual audio. Do NOT use `transcript-project-words` for caption word timings — projection from the original transcript drifts on cut/silence-stripped files. (`transcript-project-words` is still available for other uses like `framer-filter --llm` if needed, but not for captions.)
- **Always transcribe the FINAL MP4 for captions.** If reframing is applied, transcribe the `.reframed.mp4` — not the pre-reframe version. Reframing re-encodes per-scene and concatenates, introducing cumulative timing drift (~7ms/scene). Using pre-reframe transcript causes captions to desync by the end.
- **Caption pipeline:** `transcribe-source` (on the final MP4 — reframed if applicable) → `transcript-to-words` → `caption-plan` (deterministic; style preset + overrides; builds phrases from words; optional `--banner <png>` + alignment/scale flags; optional `--title`) → `video-caption-render` (single Remotion pass burns captions + optional title + optional banner). Refine via `caption-refine` → writes `.caption.N.json`, then re-render.
